...
namespace AndrejKrizan.Common.ValueObjects
{
    public class Page<T>
    {
        ...
        public Page(IReadOnlyCollection<T> results, ulong totalCount, uint pageSize)
            : this(results.ToImmutableArray(), totalCount, pageSize) { }
        ...
        public async Task<Page<T2>> ConvertAsync<T2>(
            Func<T, Task<T2>> asyncConverter, bool concurrently = false,
            CancellationToken cancellationToken = default
        )
            => new Page<T2>(await Results.ConvertAsync(asyncConverter, concurrently, cancellationToken), TotalCount, PageSize);
